#!/usr/bin/env bash
# PisimpleShot 安装脚本：复制程序/图标/启动项，并立即启动托盘图标
set -e
...
install -m 755 "$SRC_DIR/src/pisimpleshot" "$BIN/pisimpleshot"
install -m 644 "$SRC_DIR/icons/pisimpleshot.svg" "$ICON_SCAL/pisimpleshot.svg"
install -m 644 "$SRC_DIR/icons/pisimpleshot.png" "$ICON_48/pisimpleshot.png"
...
Type=Application
Version=1.0
Name=PisimpleShot 快速截图
GenericName=Screenshot Tool
Comment=在面板右上角托盘显示快速截图图标
Exec=$BIN/pisimpleshot
TryExec=$BIN/pisimpleshot
Terminal=false
Categories=Utility;
Icon=pisimpleshot
...
# ---- 4. 重启托盘程序 ----
echo "==> 启动托盘图标"
pkill -f "/.local/bin/pisimpleshot" 2>/dev/null && sleep 1 || true
...
if [ -n "$WAYLAND_DISPLAY" ]; then
    (setsid nohup "$BIN/pisimpleshot" >/dev/null 2>&1 &)
    sleep 2
    if pgrep -f "/.local/bin/pisimpleshot" >/dev/null; then
        echo "✔ 安装完成，托盘图标已启动（右上角相机图标）"
    else
        echo "⚠ 程序已安装但启动失败，请重新登录桌面或手动运行: $BIN/pisimpleshot"
        exit 1
    fi
else
